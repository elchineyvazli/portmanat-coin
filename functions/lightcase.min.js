    ...
    var f = {
        ...
        init: function (e) {
            return this.each(function () {
                r(this).unbind("click.lightcase").bind("click.lightcase", function (t) {
                    t.preventDefault(), r(this).lightcase("start", e)
                })
            })
        },
        start: function (t) {
            f.origin = lightcase.origin = this, f.settings = lightcase.settings = r.extend(!0, {
                idPrefix: "lightcase-",
                ...
                onClose: {},
                onCleanup: {}
            }, t, f.origin.data ? f.origin.data("lc-options") : {}), f.objects.document = r("html"), f.objects.body = r("body"), f._callHooks(f.settings.onInit), f.objectData = f._setObjectData(this), f._addElements(), f._open(), f.dimensions = f.getViewportDimensions()
        },
        get: function (t) {
            return f.objects[t]
        },
        ...
        _setObjectData: function (t) {
            var e = r(t),
                i = {
                    this: r(t),
                    title: f.settings.title || e.attr(f._prefixAttributeName("title")) || e.attr("title"),
                    caption: f.settings.caption || e.attr(f._prefixAttributeName("caption")) || e.children("img").attr("alt"),
                    url: f._determineUrl(),
                    requestType: f.settings.ajax.type,
                    requestData: f.settings.ajax.data,
                    requestDataType: f.settings.ajax.dataType,
                    rel: e.attr(f._determineAttributeSelector()),
                    type: f.settings.type || f._verifyDataType(f._determineUrl()),
                    isPartOfSequence: f.settings.useAsCollection || f._isPartOfSequence(e.attr(f.settings.attr), ":"),
                    isPartOfSequenceWithSlideshow: f._isPartOfSequence(e.attr(f.settings.attr), ":slideshow"),
                    currentIndex: r(f._determineAttributeSelector()).index(e),
                    sequenceLength: r(f._determineAttributeSelector()).length
                };
        ...
        _determineAttributeSelector: function () {
            var t = r(f.origin),
                i = "";
            if (void 0 !== f.cache.selector) i = f.cache.selector;
            else if (!0 === f.settings.useCategories && t.attr(f._prefixAttributeName("categories"))) {
                var e = t.attr(f._prefixAttributeName("categories")).split(" ");
                r.each(e, function (t, e) {
                    0 < t && (i += ","), i += "[" + f._prefixAttributeName("categories") + '~="' + e + '"]'
                })
            } else i = "[" + f.settings.attr + '="' + t.attr(f.settings.attr) + '"]';
            return f.cache.selector = i
        },
        _determineUrl: function () {
            var n, t = f._verifyDataUrl(f._determineLinkTarget()),
                a = 0,
                o = 0,
                c = "";
            return r.each(t, function (t, e) {
                switch (f._verifyDataType(e.url)) {
                    case "video":
                        var i = document.createElement("video"),
                            s = f._verifyDataType(e.url) + "/" + f._getFileUrlSuffix(e.url);
                        "probably" !== c && c !== i.canPlayType(s) && "" !== i.canPlayType(s) && (c = i.canPlayType(s), n = e.url);
                        break;
                    default:
                        f._devicePixelRatio() >= e.density && e.density >= o && f._matchMedia()("screen and (min-width:" + e.width + "px)").matches && e.width >= a && (a = e.width, o = e.density, n = e.url)
                }
            }), n
        },
        ...
        _createObject: function () {
            ...
            switch (f.objectData.type) {
                case "image":
                    (i = r(new Image)).attr({
                        src: f.objectData.url,
                        alt: f.objectData.title
                    });
                    break;
                case "inline":
                    (i = r('<div class="' + f.settings.classPrefix + 'inlineWrap"></div>')).html(f._cloneObject(r(f.objectData.url))), r.each(f.settings.inline, function (t, e) {
                        i.attr(f._prefixAttributeName(t), e)
                    });
                    break;
                case "ajax":
                    i = r('<div class="' + f.settings.classPrefix + 'inlineWrap"></div>'), r.each(f.settings.ajax, function (t, e) {
                        "data" !== t && i.attr(f._prefixAttributeName(t), e)
                    });
                    break;
                case "flash":
                    i = r('<embed src="' + f.objectData.url + '" type="application/x-shockwave-flash"></embed>'), r.each(f.settings.flash, function (t, e) {
                        i.attr(t, e)
                    });
                    break;
                case "video":
                    (i = r("<video></video>")).attr("src", f.objectData.url), r.each(f.settings.video, function (t, e) {
                        i.attr(t, e)
                    });
                    break;
                default:
                    (i = r("<iframe></iframe>")).attr({
                        src: f.objectData.url
        ...
        _loadObject: function (s) {
            switch (f.objectData.type) {
                case "inline":
                    ...
                    break;
                case "ajax":
                    r.ajax(r.extend({}, f.settings.ajax, {
                ...
                case "video":
                    ...
                    break;
                default:
                    f.objectData.url ? (s.on("load", function () {
                        f._showContent(s)
                    }), s.on("error", function () {
                        f.error()
                    })) : f.error()
        ...
        _adjustDimensions: function (t, e) {
            ...
            }), f.objects.contentInner.css({
                width: t.outerWidth(),
                height: t.outerHeight(),
                "max-width": "100%"
            }), f.objects.case.css({
                width: f.objects.contentInner.outerWidth(),
                "max-width": "100%"
            }), f.objects.case.css({
                "margin-top": parseInt(-f.objects.case.outerHeight() / 2),
                "margin-left": parseInt(-f.objects.case.outerWidth() / 2)
            })
        },
        _loading: function (t) {
        ...
        getViewportDimensions: function () {
            return {
                windowWidth: r(window).innerWidth(),
                windowHeight: r(window).innerHeight()
            }
        },
        ...
        _startInTransition: function () {
            ...
            f._loading("end"), f.isBusy = !1, f.cache.firstOpened || (f.cache.firstOpened = f.objectData.this), f.objects.info.hide(), setTimeout(function () {
                f.transition.fade(f.objects.info, "in", f.settings.speedIn)
            }, f.settings.speedIn), f._callHooks(f.settings.onFinish)
        },
        _processContent: function () {
            switch (f.isBusy = !0, f.transition.fade(f.objects.info, "out", 0), f.settings.transitionOut) {
                ...
                case "scrollVertical":
                case "scrollHorizontal":
                    f.objects.case.is(":hidden") ? (f.transition.fade(f.objects.contentInner, "out", 0), f.transition.fade(f.objects.case, "out", 0, 0, function () {
                        f._loadContent()
                    ...
                    break;
                case "fade":
                    f.objects.case.is(":hidden") ? f.transition.fade(f.objects.case, "out", 0, 0, function () {
                        f._loadContent()
        ...
        _handleEvents: function () {
            f._unbindEvents(), f.objects.nav.children().not(f.objects.close).hide(), f.isSlideshowEnabled() && (!0 !== f.settings.slideshowAutoStart && !f.isSlideshowStarted || f.objects.nav.hasClass(f.settings.classPrefix + "paused") ? f._stopTimeout() : f._startTimeout()), f.settings.liveResize && f._watchResizeInteraction(), f.objects.close.click(function (t) {
                t.preventDefault(), f.close()
            }), !0 === f.settings.closeOnOverlayClick && f.objects.overlay.css("cursor", "pointer").click(function (t) {
                t.preventDefault(), f.close()
            }), !0 === f.settings.useKeys && f._addKeyEvents(), f.objectData.isPartOfSequence && (f.objects.nav.attr(f._prefixAttributeName("ispartofsequence"), !0), f.objects.nav.data("items", f._setNavigation()), f.objects.prev.click(function (t) {
        ...
        _startTimeout: function () {
            f.isSlideshowStarted = !0, f.objects.play.hide(), f.objects.pause.show(), f.cache.action = "next", f.objects.nav.removeClass(f.settings.classPrefix + "paused"), f.timeout = setTimeout(function () {
        ...
        _setNavigation: function () {
            var t = r(f.cache.selector || f.settings.attr),
                ...
                i = {
                    prev: t.eq(f.objectData.prevIndex),
                    next: t.eq(f.objectData.nextIndex)
                };
        ...
        transition: {
            ...
            fade: function (t, e, i, s, n) {
                var a = "in" === e,
                    ...
                    r = {},
                    l = s || (a ? 1 : 0);
                !f.isOpen && a || (o.opacity = c, r.opacity = l, t.css(f.support.transition + "transition", "none"), t.css(o).show(), f.support.transitions ? (r[f.support.transition + "transition"] = i + "ms ease", setTimeout(function () {
                    t.css(r), setTimeout(function () {
                        t.css(f.support.transition + "transition", ""), !n || !f.isOpen && a || n()
                    }, i)
                }, 15)) : (t.stop(), t.animate(r, i, n)))
            },
            scroll: function (t, e, i, s) {
                ...
                if (f.isOpen || !n) {
                    ...
                    if ("prev" === f.cache.action) switch (a) {
                        case "scrollHorizontal":
                    ...
                    c.opacity = r, c[o] = l, d.opacity = u, d[o] = h, t.css(f.support.transition + "transition", "none"), t.css(c).show(), f.support.transitions ? (d[f.support.transition + "transition"] = i + "ms ease", setTimeout(function () {
                        t.css(d), setTimeout(function () {
                            t.css(f.support.transition + "transition", ""), !s || !f.isOpen && n || s()
                        }, i)
                    }, 15)) : (t.stop(), t.animate(d, i, s))
                }
            },
            zoom: function (t, e, i, s) {
                ...
                !f.isOpen && n || (a.opacity = o, a[f.support.transition + "transform"] = c, r.opacity = l, t.css(f.support.transition + "transition", "none"), t.css(a).show(), f.support.transitions ? (r[f.support.transition + "transform"] = d, r[f.support.transition + "transition"] = i + "ms ease", setTimeout(function () {
                    t.css(r), setTimeout(function () {
                        t.css(f.support.transition + "transform", ""), t.css(f.support.transition + "transition", ""), !s || !f.isOpen && n || s()
                    }, i)
                }, 15)) : (t.stop(), t.animate(r, i, s)))
            }
        },
        ...
        _open: function () {
            switch (f.isOpen = !0, f.support.transitions = !!f.settings.cssTransitions && f.isTransitionSupported(), f.support.mobileDevice = f.isMobileDevice(), f.support.mobileDevice && (r("html").addClass(f.settings.classPrefix + "isMobileDevice"), f.settings.fullScreenModeForMobile && f._switchToFullScreenMode()), f.settings.transitionIn || (f.settings.transitionIn = f.settings.transition), f.settings.transitionOut || (f.settings.transitionOut = f.settings.transition), f.transition.in()) {
                ...
                case "scrollVertical":
                case "scrollHorizontal":
                    f.objects.case.is(":hidden") && (f.objects.close.css("opacity", 0), f.objects.overlay.css("opacity", 0), f.objects.case.css("opacity", 0), f.objects.contentInner.css("opacity", 0)), f.transition.fade(f.objects.overlay, "in", f.settings.speedIn, f.settings.overlayOpacity, function () {
        ...
        _cleanupDimensions: function () {
            ...
            f.objects.case.css({
                ...
                "margin-top": "",
                "margin-left": ""
            }), f.objects.contentInner.removeAttr("style").css("opacity", t), f.objects.contentInner.children().removeAttr("style")
        },
        cleanup: function () {
        ...
        _export: function () {
            window.lightcase = {}, r.each(f, function (t) {
                f._isPublicMethod(t) && (lightcase[t] = f[t])
            })
        }
    };
    f._export(), r.fn.lightcase = function (t) {
